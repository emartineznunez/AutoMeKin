...
maxsteps = 500 # default for maxsteps (if not entered)
kick = 0.05 # angstroms
...
inp = open('../../amk.dat', "r")
for line in inp:
    if search("IRCpoints ", line):
        maxsteps = int(line.split()[1])
...
for i,line in enumerate(lines):
    ...
    elif line.find("Lowest normal mode") != -1:
       imag_nm = [float(line.split()[0])
              for line in lines[i + 1: i + 1 + 3 * natom ]] 
       imag_nm = np.array(imag_nm)
#We first determine e0 and the eigenvector of the imaginary freq
gradfile = name+'_grad.dat'
gradxyz  = name+'_grad.xyz'
forward=name+'_forward.xyz'
reverse=name+'_reverse.xyz'
forward_last=name+'_forward_last.xyz'
reverse_last=name+'_reverse_last.xyz'
write(forward,rmol)
write(reverse,rmol)
...
#Velocity damping
        ...
        fin4 = i > maxsteps
        if fin1 or fin2 or fin3 or fin4:
            ...
            if fin4: print('END of this path: steps >',maxsteps)
            if sign == -1:
                write(forward_last,rmol)
            else:
                write(reverse_last,rmol)
